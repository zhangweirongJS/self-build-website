...
var ZYFILE = {
    ...
    bindEvent: function(x, e_, f, e) {
        if (x.addEventListener) { //所有主流浏览器，除了 IE 8 及更早 IE版本
            x.addEventListener(e_, f, false);
        } else if (x.attachEvent) { // IE 8 及更早 IE 版本
            x.attachEvent("on" + e_, f);
        }
    },
    /* 提供给外部的接口 */
    ...
    funGetFiles: function(e) {
        ...
        // 取消鼠标经过样式
        // this.funDragHover(e);
        // 从事件中获取选中的所有文件e.dataTransfer.files;
        var files = e.target.files || e.srcElement.files;
        self.lastUploadFile = this.uploadFile;

        self.uploadFile = self.uploadFile.concat(this.filterFile(files));
        var tmpFiles = [];
    ...
    funUploadFiles: function() {
        ...
        if (this.uploadFile.length > self.maximum) {
            window.parent.Ev.pubFun.alertWindow('最多允许上传' + self.maximum + '个文件', 'error');
            return false;
        }
    ...
    funUploadFile: function(file) {
        ...
        // 绑定上传事件
        // 进度
        self.bindEvent(xhr.upload, 'progress', function(e) {
            // 回调到外部
            self.onProgress(file, e.loaded, e.total);
        });

        // 完成
        self.bindEvent(xhr, 'load', function(e) {
            ...
            if(self.uperrloadFile.length == self.uploadFile.length){
                window.parent.Ev.pubFun.delLoadingWindow();
            }
        });

        // 错误
        self.bindEvent(xhr, 'error', function(e) {
            self.uperrloadFile.push(file);
            // 回调到外部
    ...
    init: function() { // 初始化方法，在此给选择、上传按钮绑定事件
        ...
        if (this.dragDrop) {
            self.bindEvent(this.dragDrop, "dragover", function(e) {
                self.funDragHover(e);
            });
            self.bindEvent(this.dragDrop, "dragleave", function(e) {
                self.funDragHover(e);
            });
            self.bindEvent(this.dragDrop, "drop", function(e) {
                self.funGetFiles(e);
            });
        }
        ...
        if (self.fileInput) {
            // 绑定change事件
            self.bindEvent(this.fileInput, 'change', function(e) {
                // var e = e || event;
        ...
        if (self.uploadInput) {
            // 绑定click事件
            self.bindEvent(this.uploadInput, "click", function(e) {
                if ($("#" + self.waterSet)[0].checked) {
                    self.url += '&pic_water=1';
                } else {
                    self.url.replace('&pic_water=1', '');
                }
        ...
        return this.each(function() {
            ...
            this.createHtml = function() {
                ...
                if (para.dragDrop) {
                    ...
                    html += '				<span class="upImgText">';
                    if (para.bestWidth && para.bestHeight) {
                        html += '			最适合图片尺寸，宽' + para.bestWidth + 'px 高' + para.bestHeight + 'px；';
                    }
                    html += '				最多允许上传' + para.maximum + '个文件；单张不超过' + para.mFileSize + 'M</span>';
                    html += '				<div class="btns">';
                    ...
                    html += '							<img class="upload_image" src="/js/zyupload/skins/images/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '						</div>';
                    html += '					</a>';
                    html += '				</div>';
                    html += '			</div>';
            ...
            this.funFilterEligibleFile = function(files) {
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    if (this.checkFile(file)) {
                        //允许文件上传数量
                        if ((ZYFILE.uploadFile.length + arrFiles.length + 1) > para.maximum) {
                            flag = true;
                        } else {
                            // 在这里需要判断当前所有文件中
                            arrFiles.push(file);
                ...
                if (flag) {
                    window.parent.Ev.pubFun.alertWindow('最多允许上传' + para.maximum + '个文件', 'error');
                }
                return arrFiles;
            };
                ...
                if (newStr.split(".")[0] != null) {
                    ...
                    if (jQuery.inArray(type.toLowerCase(), para.fileType) > -1) {
                        ...
                        if (file.size >= para.fileSize) {
                            window.parent.Ev.pubFun.alertWindow('您这个"' + file.name + '"文件大小过大', 'error');
                            return false;
                        }
                    } else {
                        window.parent.Ev.pubFun.alertWindow('您这个"' + file.name + '"上传类型不符合', 'error');
                        return false;
                    }
                } else {
                    window.parent.Ev.pubFun.alertWindow('您这个"' + file.name + '"没有类型, 无法识别', 'error');
                    return false;
                }
                return true;
            };
                ...
                var params = {
                    ...
                    onSuccess: function(file, response) {
                        ...
                        if (parseInt(response.errorcode)) {
                            ...
                            $("#uploadFailure_" + file.index).hide();
                        }
            ...
            this.addEvent = function() {
                ...
                $(".upload_btn").bind("click", function(e) {
                    ...
                    if (ZYFILE.funReturnNeedFiles().length > 0) {
                        ...
                        window.parent.Ev.pubFun.alertWindow("请先选择文件再点击上传", 'error');
                    }
                });
