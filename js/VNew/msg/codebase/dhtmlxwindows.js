...
function dhtmlXWindows() {
	...
	this.createWindow = function(id, x, y, width, height) {
		...
		win.winZZDiv = document.createElement("DIV");
		win.winZZDiv.className = 'dhtmlx_window_zz_bg';
		win.className = "dhtmlx_window_inactive";
		win.dir = "ltr";
		this.zIndexStep = this.getZIndex();
		for (var a in this.wins) {
			this.wins[a].zi += this.zIndexStep;
			this.wins[a].style.zIndex = this.wins[a].zi;
		};
		win.zi = this.zIndexStep;
		win.winZZDiv.style.zIndex = win.zi
		win.style.zIndex = win.zi + 1;
		win.zi = win.zi + 1;
		win.active = false;
		win._isWindow = true;
		win.isWindow = true;
		...
		win.idd = id;
		this.vp.appendChild(win.winZZDiv);
		...
		win.setDimension = function(width, height) {
			if (width != null) {
				if (this.maxW != "auto")
					if (width > this.maxW) width = this.maxW;
				if (width < this.minW) width = this.minW;
				this.w = width;
			};
			if (height != null) {
				if (this.maxH != "auto")
					if (height > this.maxH) height = this.maxH;
	...
	this.zIndexStep = Math.max.apply(null, $.map($('body > *'), function(e, n) {
		var z = parseInt($(e).css('z-index'));
		if(z == 2147483647){
			z = 1000;
		}
	...
	this._parkWindow = function(win, parkBeforeMinMax) {
		...
		if (win._isParked) {
			if (this.enableParkEffect && !parkBeforeMinMax) {
				...
				win._isParked = false;
				if (!parkBeforeMinMax)
					if (win.checkEvent("onParkDown")) win.callEvent("onParkDown", [win]);
					else this.callEvent("onParkDown", [win]);
			}
		} else {
			if (this.enableParkEffect && !parkBeforeMinMax) {
				...
				win._isParked = true;
				if (!parkBeforeMinMax)
					if (win.checkEvent("onParkUp")) win.callEvent("onParkUp", [win]);
					else this.callEvent("onParkUp", [win]);
			}
		}
	};
	...
	this._maximizeWindow = function(win) {
		...
		if (win.checkEvent("onMaximize")) win.callEvent("onMaximize", [win]);
		else this.callEvent("onMaximize", [win]);
	};
	this._restoreWindow = function(win) {
		...
		if (win.checkEvent("onMinimize")) win.callEvent("onMinimize", [win]);
		else this.callEvent("onMinimize", [win]);
	};
	...
	this._removeWindowGlobal = function(win) {
		...
		win._content = null;
		win.innerHTML = "";
		win.parentNode.removeChild(win.winZZDiv);
...
dhtmlXWindows.prototype._dhx_Engine = function() {
	...
	this._engineSetWindowBody = function(win) {
		win.innerHTML = "<div iswin='1' class='dhtmlx_wins_body_outer' style='position: relative;'>" +
			...
			"<div class='dhtmlx_wins_icon' style='display:none'></div>" +
			"<div class='dhtmlx_wins_progress' style='display:none'></div>" +
			...
			"<div winResT='yes' class='dhtmlx_wins_resizer_t'><div class='dhtmlx_wins_resizer_t_l'></div><div class='dhtmlx_wins_resizer_t_r'></div><div class='dhtmlx_wins_resizer_t_c'></div></div>" +
			"<div winResL='yes' class='dhtmlx_wins_resizer_l'></div>" +
			"<div winResR='yes' class='dhtmlx_wins_resizer_r'></div>" +
			"<div winResB='yes' class='dhtmlx_wins_resizer_b'><div class='dhtmlx_wins_resizer_b_l'></div><div class='dhtmlx_wins_resizer_b_r'></div><div class='dhtmlx_wins_resizer_b_c'></div></div>" +
	...
	this._engineRedrawWindowTitle = function(win) {
		if (win._noHeader !== true) {
			var p2 = win.childNodes[0].childNodes[(_isIE ? 1 : 0)].offsetWidth;
			var p3 = win.childNodes[0].childNodes[(_isIE ? 4 : 3)].offsetWidth;
