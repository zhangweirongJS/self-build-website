...
function dhtmlXContainer(obj) {
	...
	this.obj.adjustContent = function(parentObj, offsetTop, marginTop, notCalcWidth, offsetBottom) {
		...
		if (notCalcWidth !== true)
			if (this.vs[this.av].dhxcont.offsetWidth > cw) this.vs[this.av].dhxcont.style.width = Math.max(0, cw * 2 - this.vs[this.av].dhxcont.offsetWidth) + "px";
		...
		if (offsetBottom)
			if (!isNaN(offsetBottom)) this.vs[this.av].dhxcont.style.height = Math.max(0, parseInt(this.vs[this.av].dhxcont.style.height) - offsetBottom) + "px";
	...
	this.obj.attachStatusBar = function() {
		...
		if (this.cv == this.av) this.vs[this.av].dhxcont.insertBefore(sbObj, this.vs[this.av].dhxcont.childNodes[this.vs[this.av].dhxcont.childNodes.length - 1]);
		else that.st.appendChild(sbObj);
	...
	this.obj.attachMenu = function(skin) {
		...
		if (this.cv == this.av) this.vs[this.av].dhxcont.insertBefore(menuObj, this.vs[this.av].dhxcont.childNodes[0]);
		else that.st.appendChild(menuObj);
	...
	this.obj.attachToolbar = function(skin) {
		...
		if (this.cv == this.av) this.vs[this.av].dhxcont.insertBefore(toolbarObj, this.vs[this.av].dhxcont.childNodes[(this.vs[this.av].menu != null ? 1 : 0)]);
		else that.st.appendChild(toolbarObj);
	...
	this.obj.attachGrid = function() {
		if (this._isWindow && this.skin == "dhx_skyblue") {
			this.vs[this.av].dhxcont.mainCont[this.av].style.border = "#ffffff 0px solid";
			this._redraw();
		};
	...
	this.obj._dhxContDestruct = function() {
		...
		that = null;
		if (dhtmlx.detaches)
			for (var a in dhtmlx.detaches) dhtmlx.detaches[a](this);
	};
	if (dhtmlx.attaches)
		for (var a in dhtmlx.attaches) this.obj[a] = dhtmlx.attaches[a];
};
//v.2.6 build 100722
